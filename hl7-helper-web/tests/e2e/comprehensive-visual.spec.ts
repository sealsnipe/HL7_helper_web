...
async function fillAndWaitForParse(page: any, text: string) {
...
test.describe('Comprehensive Visual Review', () => {
    ...
    for (const theme of THEMES) {
      test(`Theme: ${theme}`, async ({ page }) => {
        ...
        for (let i = 0; i < maxClicks; i++) {
          const themeButton = await page.locator('button[aria-label*="theme"], button:has-text("Theme"), button:has([class*="sun"]), button:has([class*="moon"])').first();
  ...
  test.describe('Templates List Page (/templates)', () => {
    ...
    test('Create template modal', async ({ page }) => {
      ...
      // Click create button
      const createBtn = await page.locator('button:has-text("Create"), button:has-text("New")').first();
